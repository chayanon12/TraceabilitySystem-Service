...
module.exports.searchFactory = async function (req, res) {
  try {
    ...
    query = ` SELECT * FROM "Traceability".trc_002_product_master_search('${Factory}', '${Product}'); `;
...
module.exports.deleteProduct_Master = async function (req, res) {
  try {
    const { p_tpm_factory, p_tpm_product_name } = req.body;
    query = `
       CALL "Traceability".trc_002_product_master_delete('${p_tpm_factory}', '${p_tpm_product_name}')
    `;
    const client = await ConnectPG_DB();
    const result = await client.query(query);
    await DisconnectPG_DB(client);
